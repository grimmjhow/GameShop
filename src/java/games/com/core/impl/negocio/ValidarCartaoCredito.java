 ...
 * @author Felipe
 */
import games.com.br.domain.Cartao;
...
public class ValidarCartaoCredito implements IStrategy
{
    ...
    {
        TransacaoCartao trsCartao = (TransacaoCartao) entidade;
        Cartao cartao = trsCartao.getCartao();

        try
        {
            if(validCC(cartao.getNumeroCartao()))
                return null;
            else
                return "Numero do Cartão Inválido";
        }
        catch(Exception ex)
        {
            ex.printStackTrace();
            return "Erro ao processar cartao "+ex.getMessage();
        }
    }
}
